...
namespace AsciiChart.Sharp
{
    public static class AsciiChart
    {
        ...
        public static string Plot(IEnumerable<IEnumerable<double>> data, Options options = null)
        {
            ...
            var max = dataList.SelectMany(s => s).Where(v => !double.IsNaN(v)).Max();

            // Fixed Y bounds (sticky expand-only envelope)
            if (options.LowerBound.HasValue)
                min = options.LowerBound.Value;
            if (options.UpperBound.HasValue)
                max = options.UpperBound.Value;
            if (max < min)
            {
                var tmpB = min;
                min = max;
